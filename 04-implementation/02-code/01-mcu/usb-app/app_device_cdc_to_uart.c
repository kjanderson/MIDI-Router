...
#define mDataRdyUSART() 1U
#define mTxRdyUSART() 1U
...
void APP_DeviceCDCEmulatorTasks()
{
    ...
    if(RS232_Out_Data_Rdy)
    {
        ...
        if ((NextUSBOut < CDC_DATA_OUT_EP_SIZE) &&
                (RS232cp < CDC_DATA_IN_EP_SIZE)) {
            USB_Out_Buffer[NextUSBOut] = RS232_Out_Data[RS232cp];
        }
        NextUSBOut++;
        RS232cp++;
        if (RS232cp == LastRS232Out) {
